...
public class BongItem extends SmokeItem {
    ...
    protected ItemStack afterSmokeUse(ItemStack stack, Level level, Player player) {
        if (!player.getAbilities().instabuild) {
            ...
            if (nextDamage >= maxUsableDamage) {
                player.displayClientMessage(Component.translatable("message.weed.bong_empty"), true);
            }
        }
        return stack;
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
            ...
            if (!level.isClientSide()) {
                player.openMenu(
                        new SimpleMenuProvider((containerId, inventory, ignoredPlayer) -> new BongControlMenu(containerId, inventory, hand),
                                Component.translatable("screen.weed.bong_control")),
        ...
        if (!player.getAbilities().instabuild && isEmpty(stack)) {
            if (!level.isClientSide()) {
                player.displayClientMessage(Component.translatable("message.weed.bong_need_refill"), true);
            }
        ...
        if (!isLoaded(stack)) {
            if (!level.isClientSide()) {
                player.displayClientMessage(Component.translatable("message.weed.bong_open_menu"), true);
            }
            return InteractionResult.FAIL;
        }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (stack.getDamageValue() == 0) {
            player.displayClientMessage(Component.translatable("message.weed.bong_already_full"), true);
            return InteractionResult.SUCCESS;
        }
        ...
        if (WeedConfig.isSmokeParticlesEnabled() && level instanceof ServerLevel serverLevel) {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay tooltipDisplay, Consumer<Component> tooltipAdder, TooltipFlag flag) {
        ...
        tooltipAdder.accept(Component.translatable("tooltip.weed.bong_water", waterLeft, maxUsableDamage));

        Component loadedState = isLoaded(stack) ? Component.translatable("hud.weed.loaded") : Component.translatable("hud.weed.empty");
        tooltipAdder.accept(Component.translatable("tooltip.weed.loaded_status", loadedState));
