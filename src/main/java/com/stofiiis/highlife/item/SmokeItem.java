...
public class SmokeItem extends Item {
    ...
    protected void applySmokeEffects(Player player, boolean hadRelaxedBeforeUse, StrainData strainData) {
        ...
        float cottonmouthChance = Mth.clamp(0.78F * HighLifeConfig.getCottonmouthChanceMultiplier() * harshnessMultiplier, 0.0F, 1.0F);
        ...
        float fogChance = Mth.clamp((0.5F + 0.15F * potency) * HighLifeConfig.getFogChanceMultiplier() * harshnessMultiplier, 0.0F, 1.0F);
    ...
    protected void emitSmokeFeedback(Level level, Player player, ItemStack stack) {
        if (HighLifeConfig.isSmokeSoundsEnabled()) {
