...
public class PipeItem extends SmokeItem {
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
            ...
            if (!level.isClientSide()) {
                player.openMenu(
                        new SimpleMenuProvider((containerId, inventory, ignoredPlayer) -> new PipeControlMenu(containerId, inventory, hand),
                                Component.translatable("screen.highlife.pipe_control")),
        ...
        if (!level.isClientSide()) {
            if (player.getCooldowns().isOnCooldown(stack)) {
                ...
                player.displayClientMessage(Component.translatable("screen.highlife.pipe_cooldown", cooldown), true);
            } else {
                player.displayClientMessage(Component.translatable("message.highlife.pipe_open_menu"), true);
            }
        }
    ...
    protected ItemStack afterSmokeUse(ItemStack stack, Level level, Player player) {
        if (!player.getAbilities().instabuild) {
            ...
            player.displayClientMessage(Component.translatable("message.highlife.pipe_empty"), true);
        }
        return stack;
    }
    ...
    public static void setLoadedFromMenu(ItemStack stack, StrainData strainData, Player player) {
        ...
        player.displayClientMessage(Component.translatable("message.highlife.pipe_loaded"), true);
