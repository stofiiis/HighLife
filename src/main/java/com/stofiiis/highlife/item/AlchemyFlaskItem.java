...
public class AlchemyFlaskItem extends InfusionItem {
    private static final String TAG_LOADED = "alchemy_flask_loaded";
    private static final String TAG_LOADED_STRAIN = "alchemy_flask_loaded_strain";
    private static final String TAG_LOADED_QUALITY = "alchemy_flask_loaded_quality";

    public AlchemyFlaskItem(Properties properties, float potency, int serenityDuration, int thirstDuration, int hazeDuration, int toleranceGain) {
    ...
    protected ItemStack afterInfusionUse(ItemStack stack, Level level, Player player) {
        if (!player.getAbilities().instabuild) {
            ...
            if (nextDamage >= maxUsableDamage) {
                player.displayClientMessage(Component.translatable("message.highlife.alchemy_flask_empty"), true);
            }
        }
        return stack;
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        if (player.isShiftKeyDown()) {
            if (!level.isClientSide()) {
                player.openMenu(
                        new SimpleMenuProvider((containerId, inventory, ignoredPlayer) -> new AlchemyFlaskControlMenu(containerId, inventory, hand),
                                Component.translatable("screen.highlife.alchemy_flask_control")),
        ...
        if (!player.getAbilities().instabuild && isEmpty(stack)) {
            if (!level.isClientSide()) {
                player.displayClientMessage(Component.translatable("message.highlife.alchemy_flask_need_refill"), true);
            }
        ...
        if (!isLoaded(stack)) {
            if (!level.isClientSide()) {
                player.displayClientMessage(Component.translatable("message.highlife.alchemy_flask_open_menu"), true);
            }
            return InteractionResult.FAIL;
        }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (stack.getDamageValue() == 0) {
            player.displayClientMessage(Component.translatable("message.highlife.alchemy_flask_already_full"), true);
            return InteractionResult.SUCCESS;
        }
        ...
        if (HighLifeConfig.isInfusionParticlesEnabled() && level instanceof ServerLevel serverLevel) {
        ...
        player.displayClientMessage(Component.translatable("message.highlife.alchemy_flask_refilled"), true);
        AdvancementTracker.onFlaskRefilled(player);
        return InteractionResult.SUCCESS;
    }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay tooltipDisplay, Consumer<Component> tooltipAdder, TooltipFlag flag) {
        ...
        tooltipAdder.accept(Component.translatable("tooltip.highlife.alchemy_flask_water", waterLeft, maxUsableDamage));
    ...
    public static void setLoadedFromMenu(ItemStack stack, StrainData loadedStrain, Player player) {
        ...
        player.displayClientMessage(Component.translatable("message.highlife.alchemy_flask_loaded"), true);
        AdvancementTracker.onFlaskLoaded(player);
    }
