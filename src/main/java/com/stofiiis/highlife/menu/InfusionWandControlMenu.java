...
public class InfusionWandControlMenu extends AbstractContainerMenu {
    ...
    public InfusionWandControlMenu(int containerId, Inventory inventory, RegistryFriendlyByteBuf data) {
        this(containerId, inventory, decodeHand(data));
    }

    public InfusionWandControlMenu(int containerId, Inventory inventory, InteractionHand hand) {
        super(ModMenus.INFUSION_WAND_CONTROL.get(), containerId);
        this.player = inventory.player;
        this.hand = hand;

        this.addSlot(new Slot(this.inputContainer, SLOT_INPUT, 44, 39) {
            @Override
        ...
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return InfusionWandControlMenu.this.loadedState;
            }

            @Override
            public void set(int value) {
                InfusionWandControlMenu.this.loadedState = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return InfusionWandControlMenu.this.cooldownPercent;
            }

            @Override
            public void set(int value) {
                InfusionWandControlMenu.this.cooldownPercent = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return InfusionWandControlMenu.this.progress;
            }

            @Override
            public void set(int value) {
                InfusionWandControlMenu.this.progress = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return InfusionWandControlMenu.this.progressMax;
            }

            @Override
            public void set(int value) {
                InfusionWandControlMenu.this.progressMax = Math.max(1, value);
            }
        });
    }
    ...
    public boolean stillValid(Player player) {
        return player.isAlive() && this.getControlledStack().getItem() instanceof InfusionWandItem;
    }
    ...
    private void tickLoading() {
        ...
        InfusionWandItem.setLoadedFromMenu(this.getControlledStack(), strainData, this.player);
    ...
    private boolean canLoad() {
        ...
        if (!(stack.getItem() instanceof InfusionWandItem)) {
            return false;
        }
        if (InfusionWandItem.isLoaded(stack)) {
            return false;
        }
        if (Math.round(InfusionWandItem.getReloadCooldownPercent(this.player, stack, 0.0F) * 100.0F) > 0) {
            return false;
        }
        return this.inputContainer.getItem(SLOT_INPUT).is(ModItems.DRIED_MYSTIC_HERB.get());
    }

    private void refreshStateFromItem() {
        ItemStack stack = this.getControlledStack();
        this.loadedState = InfusionWandItem.isLoaded(stack) ? 1 : 0;
        this.cooldownPercent = Math.round(InfusionWandItem.getReloadCooldownPercent(this.player, stack, 0.0F) * 100.0F);
    }
