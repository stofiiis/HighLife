...
public class BongControlMenu extends AbstractContainerMenu {
    ...
    public BongControlMenu(int containerId, Inventory inventory, RegistryFriendlyByteBuf data) {
        this(containerId, inventory, decodeHand(data));
    }

    public BongControlMenu(int containerId, Inventory inventory, InteractionHand hand) {
        super(ModMenus.BONG_CONTROL.get(), containerId);
        ...
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return BongControlMenu.this.loadedState;
            }

            @Override
            public void set(int value) {
                BongControlMenu.this.loadedState = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return BongControlMenu.this.waterPercent;
            }

            @Override
            public void set(int value) {
                BongControlMenu.this.waterPercent = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return BongControlMenu.this.processMode;
            }

            @Override
            public void set(int value) {
                BongControlMenu.this.processMode = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return BongControlMenu.this.progress;
            }

            @Override
            public void set(int value) {
                BongControlMenu.this.progress = value;
            }
        });
        this.addDataSlot(new DataSlot() {
            @Override
            public int get() {
                return BongControlMenu.this.progressMax;
            }

            @Override
            public void set(int value) {
                BongControlMenu.this.progressMax = Math.max(1, value);
            }
        });
    }
    ...
    public boolean stillValid(Player player) {
        return player.isAlive() && this.getControlledStack().getItem() instanceof BongItem;
    }
    ...
    private boolean canPackBowl() {
        ...
        if (!(stack.getItem() instanceof BongItem) || BongItem.isLoaded(stack)) {
            return false;
        }
        return this.inputContainer.getItem(SLOT_BUD).is(ModItems.DRIED_CANNABIS_BUD.get());
    }

    private boolean canRefillWater() {
        ItemStack stack = this.getControlledStack();
        if (!(stack.getItem() instanceof BongItem) || BongItem.getWaterRatio(stack) >= 0.999F) {
            return false;
        }
    ...
    private void finishPacking() {
        ...
        BongItem.setLoadedFromMenu(stack, strainData, this.player);
    ...
    private void finishRefill() {
        ...
        BongItem.setWaterFullFromMenu(stack, this.player);
        this.inputContainer.setChanged();
    }

    private void refreshStateFromItem() {
        ItemStack stack = this.getControlledStack();
        this.loadedState = BongItem.isLoaded(stack) ? 1 : 0;
        this.waterPercent = Math.round(BongItem.getWaterRatio(stack) * 100.0F);
    }
