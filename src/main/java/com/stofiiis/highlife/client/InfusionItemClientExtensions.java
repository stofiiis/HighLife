...
public final class InfusionItemClientExtensions {
    private InfusionItemClientExtensions() {
    }

    @SubscribeEvent
    public static void onRegisterClientExtensions(RegisterClientExtensionsEvent event) {
        event.registerItem(new InfusionPoseExtension(ModArmPoses.ROLL_CAST, 1.0F, 0.95F), ModItems.HERB_ROLL.get());
        event.registerItem(new InfusionPoseExtension(ModArmPoses.WAND_CHARGE, 1.10F, 1.10F), ModItems.INFUSION_WAND.get());
        event.registerItem(new InfusionPoseExtension(ModArmPoses.FLASK_BUBBLE, 1.22F, 1.20F), ModItems.ALCHEMY_FLASK.get());
    }

    private static final class InfusionPoseExtension implements IClientItemExtensions {
        ...
        private InfusionPoseExtension(EnumProxy<HumanoidModel.ArmPose> poseProxy, float firstPersonPull, float firstPersonTwist) {
