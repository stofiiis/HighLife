...
public class AlchemyFlaskControlScreen extends AbstractContainerScreen<AlchemyFlaskControlMenu> {
    private static final Identifier BACKGROUND_TEXTURE = Identifier.fromNamespaceAndPath(HighLifeMod.MODID, "textures/gui/alchemy_flask_control.png");

    public AlchemyFlaskControlScreen(AlchemyFlaskControlMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.drawString(this.font, Component.translatable("screen.highlife.alchemy_flask_loaded", bowlState), 8, 20, 0xD4E4EB);
        guiGraphics.drawString(this.font, Component.translatable("screen.highlife.alchemy_flask_water", this.menu.getWaterPercent()), 8, 32, 0xD4E4EB);

        String modeText = switch (this.menu.getProcessMode()) {
            case AlchemyFlaskControlMenu.MODE_PACKING -> Component.translatable("screen.highlife.packing_bowl").getString();
            case AlchemyFlaskControlMenu.MODE_REFILLING -> Component.translatable("screen.highlife.refilling_water").getString();
            default -> Component.translatable("screen.highlife.insert_alchemy_flask_items").getString();
        };
